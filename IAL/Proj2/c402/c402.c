...
**/
	...
	while (*iter != NULL)
	{
		if ((*iter)->Cont == Content)
			return;
		iter = (*iter)->Cont > Content ? &((*iter)->LPtr) : &((*iter)->RPtr);
	};
	*iter = malloc(sizeof(struct tBTNode));
	if (*iter == NULL)
		return;
	(*iter)->Cont = Content;
	(*iter)->LPtr = NULL;
...
**/
	...
	{
		tBTNodePtr currP = STopPopP(stackP);
		bool currB = STopPopB(stackB);

		if (currB) {
			SPushP(stackP, currP);
			SPushB(stackB, false);
			Leftmost_Postorder(currP->RPtr, stackP, stackB);
		}
		else {
			BTWorkOut(currP);
		}
